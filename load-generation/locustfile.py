...
class UserBehavior(HttpUser):
    ...
    # TeaStore is deployed under this context path
    CONTEXT_PATH = "/tools.descartes.teastore.webui"
    ...
    def visit_home(self) -> None:
        ...
        """
        # load landing page
        res = self.client.get(f"{self.CONTEXT_PATH}/")
        if res.ok:
    ...
    def login(self) -> None:
        ...
        """
        # load login page
        res = self.client.get(f"{self.CONTEXT_PATH}/login")
        if res.ok:
        ...
        # login random user
        user = randint(1, 99)
        # Using params as per upstream example
        login_request = self.client.post(f"{self.CONTEXT_PATH}/loginAction", params={"username": user, "password": "password"})
        if login_request.ok:
    ...
    def browse(self) -> None:
        ...
        for i in range(1, randint(2, 5)):
            ...
            page = randint(1, 5)
            category_request = self.client.get(f"{self.CONTEXT_PATH}/category", params={"page": page, "category": category_id})
            if category_request.ok:
                ...
                product_request = self.client.get(f"{self.CONTEXT_PATH}/product", params={"id": product_id})
                if product_request.ok:
                    logging.info(f"Visited product with id {product_id}.")
                    # Add to cart
                    cart_request = self.client.post(f"{self.CONTEXT_PATH}/cartAction", params={"addToCart": "", "productid": product_id})
                    if cart_request.ok:
    ...
    def buy(self) -> None:
        ...
        buy_request = self.client.post(f"{self.CONTEXT_PATH}/cartAction", params=user_data)
        if buy_request.ok:
    ...
    def visit_profile(self) -> None:
        ...
        :return: None
        """
        profile_request = self.client.get(f"{self.CONTEXT_PATH}/profile")
    ...
    def logout(self) -> None:
        ...
        :return: None
        """
        logout_request = self.client.post(f"{self.CONTEXT_PATH}/loginAction", params={"logout": ""})
